...
export function TicketPurchase({ onSuccess, disabled }: TicketPurchaseProps) {
  const { buyTicket, currentRound, error, loading, isConnected } = useLottery();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!guess || parseInt(guess) < 0 || parseInt(guess) > 99) {
      return;
    }

    const txHash = await buyTicket(parseInt(guess));
    if (txHash) {
      setGuess('');
      onSuccess?.();
    }
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      <h3 className="text-xl font-semibold text-gray-900 mb-4">Buy Lottery Ticket</h3>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <div className="relative">
            ...
            <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
              <span className="text-gray-500 text-sm">{guess ? String(parseInt(guess)).padStart(2, '0') : '--'}</span>
            </div>
          </div>
        </div>
        ...
        <button
          type="submit"
          disabled={!guess || loading || disabled}
